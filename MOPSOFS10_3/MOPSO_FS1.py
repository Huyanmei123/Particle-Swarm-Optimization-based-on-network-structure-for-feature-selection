...
initial_dir = utils.initial_dir
Dom_dir = utils.Dom_dir
Gbest_dir = utils.Gbest_dir
...
# %%
class MOPSO:
    def __init__(self, N, dim, Gm, x_min, lp, up, c1, c2, wl, wu, M, part):
        # time1 = time.time()
        self.N = N
        self.dim = dim
        self.Gm = Gm
        ...
        self.part = part
        self.x_min = x_min
        self.lp, self.up = lp, up
        self.c1, self.c2 = c1, c2
        self.wl, self.wu = wl, wu # low, upper, inertia weight
        self.Vmax=(up-lp)/2
        self.X = np.zeros((2*self.N,self.dim),dtype=float)
        # self.V = np.zeros((self.N, self.dim), dtype=float)
        self.V = np.random.random((2*self.N, self.dim))
        ...
        # self.V = np.random.random((2*N, dim)) # the velocity of particles
        # self.X = np.random.random((2*N, dim))
        # self.Ovs = np.zeros((2*N, 7))  # n old pop, n offs
        # self.Ovs[:N] = np.array([self.obj_fun(self.X[i], 0) for i in range(N)])
        # maxIndex = np.argmax(self.Ovs[:N, -1])
    ...
    def initialization_random(self, ini_p, h):
        ...
        for i in range(self.N):
            for d in range(self.dim):
                self.X[i,d] = self.lp+(self.up-self.lp)*np.random.random()
        # self.pOvs = np.array([self.obj_function(self.X[i], 0) for i in range(N)])
        # maxIndex = np.argmax(self.pOvs[:, -1])
        # self.metrics[0] = self.pOvs[maxIndex]
        # self.pbest = self.X.copy()
        # self.gbest = self.X[maxIndex].copy()
        # self.gOvs = self.pOvs[maxIndex].copy()
        self.up_ini(ini_p, h)
        time2 = time.time()
        totaltime = time2-time1
        self.timeCost[0, :2] = [totaltime, totaltime-self.timeCost[0,-1]]
    ...
    def up_ini(self, ini_p, h):
        ...
        # maxIndex = np.argmax(self.Ovs[:self.N, -1])
        ...
        # self.pOvs = np.array([self.obj_fun(self.X[i], 0) for i in range(N)])
        ...
        # self.gOvs = self.pOvs[maxIndex].copy()
        # self.minMax = np.array([np.min(self.X, axis=0), np.max(self.X, axis=0)])
        # df = pd.DataFrame(self.pOvs)
        # df.to_csv(ini_p, header=h)
        write_rows(ini_p, self.pOvs, h)
    ...
    def obj_fun(self, ith, t):  # _pos_inv 个体的一组元素值，_X DE算法的所有pos值
        ...
        # network + community
    def iterator_netg(self, ver_network, ver_indp, g, c, Alpha, Beta, Gamma, ver_com, percen):
        ...
        for t in range(1, self.Gm+1):
            ...
            for i in range(self.N):
                ...
                # offs = np.zeros(self.dim, dtype=float)
                for d in ver_indp:
                    r1,r2 = np.random.random(), np.random.random()
                    ww = np.random.random()*(self.wu-self.wl)+self.wl
                    vb = ww*self.V[i, d]+self.c1*r1*(self.pbest[i,d]-self.X[i, d])+self.c2*r2*(self.gbest[d]-self.X[i,d])
                    if vb > self.Vmax: vb=self.Vmax
                    if vb < -self.Vmax: vb = -self.Vmax
                    # self.X[i,d] += self.V[i,d]
                    self.V[newindex,d] = vb
                    self.X[newindex,d] += vb
                # 对网络节点的处理
                random.shuffle(ver_network)
                u_decode = self.indv_decode(self.X[i])
                for j in ver_network:
                    ...
                    if rand_val < pj and self.X[i, j]<self.x_min:
                        if self.V[i, j]>0:
                            self.V[newindex, j] *= math.exp(pj*percen)
                        else:
                            self.V[newindex, j] *= math.exp(-pj*percen)
                    elif rand_val >= pj and self.X[i, j]>= self.x_min:
                        if self.V[i, j]>0:
                            self.V[newindex, j] *= math.exp((pj-1)*percen)
                        else:
                            self.V[newindex, j] *= math.exp((1-pj)*percen)
                    self.X[newindex, j] += self.V[newindex, j]
                self.X[newindex, self.X[newindex, :]>self.up] = self.up
                self.X[newindex, self.X[newindex, :]<self.lp] = self.lp
    ...
    # 只有network
    def iterator_network(self, ver_network, ver_indp, g, Alpha, Beta, percen):
        # 时间开销：total time in one iteration of PSO, time_svm, pj_svm
        for t in range(1, self.Gm+1):
            # 更新velocity和position
            time1 = time.time()
    ...
    # 1. update the non-dominated solutoin set; 2. update the global best; 3. select the parents for next generation
    # def update(self, i, t):
    #     ovs = self.obj_fun(self.X[i], t)
    #     # oldi = i-self.N
    #     if ovs[-1] > self.pOvs[i, -1]:
    #         self.pOvs[i] = ovs.copy()
    #         self.pbest[i] = self.X[i].copy()
    #     if ovs[-1] > self.gOvs[-1]:
    #         self.gOvs = ovs.copy()
    #         self.gbest = self.X[i].copy()
    #         self.metrics[t] = ovs.copy()  
    ...
    def select(self):
        ...
        self.V[:N] = self.V[selectIndex].copy()  # update velocity
        self.Ovs[:N] = self.Ovs[selectIndex].copy()  # update obj vals
        # for i in range(N):
        #     r = selectIndex[i]
        #     if r % 2 == 0: # 更新个体
        #         self.X[i] = self.X[r//3].copy()
        #     elif r % 3 == 1:
        #         self.X[i] = self.U[r//3].copy()
        #     else:
        #         self.X[i] = self.V[r//3].copy()
        #     self.Ovs[2*i] = self.Ovs[r].copy() # 更新目标函数值
    ...
    def updateGbest(self):
        ...
        # probabilities = np.array([len(hypercubes[i]) for i in range(len(hypercubes))])
        ...
        if(len(hypercubes[x][y])==1):
            self.gbest = self.X[hypercubes[x][y][0]].copy()
            self.gOvs = self.Ovs[hypercubes[x][y][0]].copy()
    ...
    def update_pbest(self, i):
        # self.obj_fun(i+self.N, t) # cal objective values for offs[i]
        # flag = self.judge_relationship(i, i+self.N)
        flag1, flag2 = self.judge_relationship(self.X[i], self.X[i+self.N])
    ...
    def execution(self, dataset):
        ...
        if flag=='1':
            ...
            str1 = '{} {} weight_{} Iteration_{} N_{} Percen_{} rank_{} {}'.format(dataset, algorithm, self.weight, self.Gm, self.N, percen, rank, timestamp)
            # str1 = dataset +' ' + algorithm + ' weight_' + str(self.weight) + ' Iteration_' + str(self.Gm) + ' N_' + str(self.N) + ' percen_' + str(percen) + ' rank_' + str(rank) + ' ' + timestamp
            ...
            # Alpha, Beta = 0.5, 0.5
            str1 = '{} {} weight_{} Iteration_{} N_{} Percen_{} rank_{} {}'.format(dataset, algorithm, self.weight, self.Gm, self.N, percen, rank, timestamp)
            # str1 = dataset +' ' + algorithm + ' weight_' + str(self.weight) + ' Iteration_' + str(self.Gm) + ' N_' + str(self.N) + ' Part_' + str(part) + ' rank_' + str(rank) + ' ' + timestamp          
            init_time = self.initialization_network(self.N, g, ver_network, ver_indp, Alpha, Beta, initial_dir+'Initialization '+str1+'.csv', fit_h)
            # Ovs_p = ovs_dir + 'Ovs ' + str1 + '.csv'
            self.iterator_network(ver_network, ver_indp, g, Alpha, Beta, percen)
        else:
            ...
            # init_time = self.initialization_network(self.N, g, ver_network, ver_indp, Alpha, Beta, initial_dir+'Initialization '+str1+'.csv', fit_h)
            # Ovs_p = ovs_dir + 'Ovs ' + str1 + '.csv'
            self.iterator_netg(ver_network, ver_indp, g, c, Alpha, Beta, Gamma, ver_com, percen)
        ...
        f = open(log_dir+'log-MOPSO(10-3).txt', 'a')
        f.write(algorithm+' initialization cost '+str(init_time)+' seconds\n')
        f.close()
        write_rows(dom_path, self.Dom, fit_h)
        write_rows(gbest_path, self.metrics, fit_h)
...
if __name__ == "__main__":
    ...
    part=3
    mopso = MOPSO(N, dim, Gm, x_min, lp, up, c1, c2, wl, wu, M, part)
