...
class MOPSO:
    ...
    # network + community
    def iterator_netg(self, ver_network, g, c, Alpha, Beta, Gamma, vers, percen, dom_path, gbest_path, time_path, fit_h, time_h):
    ...
    # 只有network
    def iterator_network(self, ver_network, g, Alpha, Beta, percen, dom_path, gbest_path, time_path, fit_h, time_h):
        # 时间开销：total time in one iteration of PSO, time_svm, pj_svm
        for t in range(1, self.Gm+1):
            # 更新velocity和position
            time1 = time.time()
    ...
    def iterator_standard(self, dom_path, gbest_path, time_path, fit_h, time_h):
        N = self.N
        for t in range(1, self.Gm+1):
            time1 = time.time()
            # update the velocity and position of particles
    ...
    def execution(self, dataset):
        ...
        if flag=='1':
            ...
            self.iterator_standard(dom_path, gbest_path, time_path, fit_h, time_h)
        elif flag=='2':
            algorithm='Net-MOPSO'
            ...
            gbest_path = '{}Gbest {}.csv'.format(Gbest_dir, str1)
            time_path = '{}timeEstimation {}.csv'.format(timeEstimation_dir, str1)
            self.iterator_network(ver_network, g, Alpha, Beta, percen, dom_path, gbest_path, time_path, fit_h, time_h)
        else:
            ...
            gbest_path = '{}Gbest {}.csv'.format(Gbest_dir, str1)
            time_path = '{}timeEstimation {}.csv'.format(timeEstimation_dir, str1)
            self.iterator_netg(ver_network, g, c, Alpha, Beta, Gamma, vers, percen, dom_path, gbest_path, time_path, fit_h, time_h)
