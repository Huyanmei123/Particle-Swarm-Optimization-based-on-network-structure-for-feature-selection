...
pjs_dir = result_dir + 'pjs/'
pj_vals_indv_dir, pj_range_indv_dir, pj_range_iter_dir = pjs_dir+'pjValsIndv/', pjs_dir+'pjRangeIndv/', pjs_dir+'pjRangeIter/'
log_dir = './log/'
dirs = [result_dir, initial_dir, excels_dir, timeEstimation_dir, Figs_dir, ovs_dir, pjs_dir, pj_vals_indv_dir, pj_range_indv_dir, pj_range_iter_dir, log_dir]
...
def write_rows2(data, path, h, m, ind):
    df = pd.DataFrame({ind: data})
    # df = pd.DataFrame(data, index=ind)
    df.to_csv(path, header=h, mode=m)

def write_row(data, path):
...
class PSO:
    ...
    def iterator_standard(self, Ovs_p):
    ...
    # network + community
    def iterator_netg(self, g, c, Alpha, Beta, Gamma, ver_com, ver_net, ver_net2, ver_indp, Ovs_p, quantile, pj_p):
        pj_vals_indv_p, pj_range_indv_p, pj_range_iter_p = pj_p[0], pj_p[1], pj_p[2]
        
        # 网络结构中的节点数
        ver_net_num = len(ver_net)
        # 时间开销：total time in one iteration of PSO, time_svm, pj_svm
        for t in range(1, self.Gm+1):
            # 更新velocity和position
            time3 = time.time()
            pj_range_iter = [t, float('inf'), float('-inf')] # min, max
            for i in range(self.pN):
                # cnt1, cnt2 = 0, 0
                self.V[i] = self.w*self.V[i] + self.c1*self.r1*(self.pBest[i] - self.X[i])+ self.c2*self.r2*(self.gBest - self.X[i])
                # 计算网络结构中，所有节点的pj值，并排序，确定阈值()
                random.shuffle(ver_com)
                u_decode = self.indv_decode(self.X[i])
                pjs = np.zeros(self.dim, dtype=float)
                for j in ver_com: # 对于社区结构中的节点
                    # 第j位对应的特征节点的加权度
                    djw = g.get_weight_degree(j)
                    # 该特征节点的邻接点中被置为1的加权个数，在变异后的个体中选择元素，并在graph中找该特征节点与对应特征节点的weight，并求和
                    wjn = g.cal_weight_number(j, u_decode)
                    ...
                    pjs[j] = Alpha * math.exp(-3/djw) + Beta * math.exp(-wjn) + Gamma * math.exp(-njc)
                random.shuffle(ver_net2)
                for j in ver_net2:  # 对于在网络结构中而不在社区结构中的节点
                    # 第j位对应的特征节点的加权度
                    djw = g.get_weight_degree(j)
                    # 该特征节点的邻接点中被置为1的加权个数，在变异后的个体中选择元素，并在graph中找该特征节点与对应特征节点的weight，并求和
                    wjn = g.cal_weight_number(j, u_decode)
                    pjs[j] = 0.5 * math.exp(-3/djw) + 0.5 * math.exp(-wjn)
                write_row(np.append([t], pjs), pj_vals_indv_p)
                # 确定pj的阈值
                index = np.argsort(pjs)
                pj_range_indv = [t, pjs[index[-ver_net_num]], pjs[index[-1]]]
                write_row(pj_range_indv, pj_range_indv_p) # 写入pj的取值范围（每个个体）
                if pj_range_indv[1]<pj_range_iter[1]:
                    pj_range_iter[1] = pj_range_indv[1]
                if pj_range_indv[2]>pj_range_iter[2]:
                    pj_range_iter[2] = pj_range_indv[2]
                # 设置最高的1/5为阈值
                pj_thre = pjs[index[-int(ver_net_num/quantile)]]
                for j in ver_net:
                    if pjs[j]>=pj_thre and self.V[i, j]<0 and np.random.rand()<pj_thre:
                        self.V[i, j] = self.V[i, j]*6*math.exp(-2.1*pjs[j])/10
                        # cnt1+=1
                    elif pjs[j]<pj_thre and self.V[i, j]>0 and np.random.rand()>pj_thre:
                        self.V[i, j] = self.V[i, j]*6*math.exp(2.1*pjs[j])/82
                        # cnt2+=1
                self.X[i] += self.V[i]
                self.X[i] = self.normalization(self.X[i])
                self.update(i, t)
            write_row(pj_range_iter, pj_range_iter_p) # 每一次迭代中，pj的取值范围
            write_rows(self.Ovs, Ovs_p, None, 'a')# 把每一代的种群写入文件中
            time4 = time.time()
            total_t = time4-time3
            self.timeCost[t,:2] = [total_t, total_t - self.timeCost[t,-1]]
            self.check(t)
    ...
    # 只有network
    def iterator_network(self, g, Alpha, Beta, ver_net, ver_indp, Ovs_p, quantile, pj_p):
        pj_vals_indv_p, pj_range_indv_p, pj_range_iter_p = pj_p[0], pj_p[1], pj_p[2]
        # 网络结构中的节点数
        ver_net_num = len(ver_net)
        for t in range(1, self.Gm+1):
            ...
            for i in range(self.pN):
                ...
                u_decode = self.indv_decode(self.X[i])
                pjs = np.zeros(self.dim, dtype=float)
                random.shuffle(ver_net)
                for j in ver_net:
                    # 第j位对应的特征节点的加权度
                    djw = g.get_weight_degree(j)
                    # 该特征节点的邻接点中被置为1的加权个数，在变异后的个体中选择元素，并在graph中找该特征节点与对应特征节点的weight，并求和
                    wjn = g.cal_weight_number(j, u_decode)
                    pjs[j] = Alpha * math.exp(-3/djw) + Beta * math.exp(-wjn)
                write_row(np.append([t], pjs), pj_vals_indv_p)
                # 确定pj的阈值
                index = np.argsort(pjs)
                pj_range_indv = [t, pjs[index[-ver_net_num]], pjs[index[-1]]]
    ...
    def execution(self):
        ...
        if flag=='1':
            ...
            str1 = dataset +' ' + algorithm + ' weight_' + str(self.weight) + ' Iteration_' + str(self.Gm) + ' N_' + str(self.pN) + ' Quantile_' + str(quantile) + ' rank_' + str(rank) + ' ' + timestamp
            init_time = self.initialization_random(initial_dir+'Initialization '+str1+'.csv', fit_h)
            Ovs_p = ovs_dir + 'Ovs ' + str1 + '.csv'
            self.iterator_standard(Ovs_p)
        elif flag=='2':
            algorithm='Net-PSO'
            ...
            Ovs_p = ovs_dir + 'Ovs ' + str1 + '.csv'
            pj_vals_indv_p, pj_range_indv_p, pj_range_iter_p = '{}pjValsIndv {}.csv'.format(pj_vals_indv_dir, str1), '{}pjRangeIndv {}.csv'.format(pj_range_indv_dir, str1), '{}pjRangeIter {}.csv'.format( pj_range_iter_dir, str1)
            pjs_p = [pj_vals_indv_p, pj_range_indv_p, pj_range_iter_p]
            self.iterator_network(g, Alpha, Beta, ver_network, ver_indp, Ovs_p, quantile, pjs_p)
        else:
            algorithm = 'NetG-PSO'
            g = Graph.Graph(path_network, self.weight)
            ...
            pj_vals_indv_p, pj_range_indv_p, pj_range_iter_p = '{}pjValsIndv {}.csv'.format(pj_vals_indv_dir, str1), '{}pjRangeIndv {}.csv'.format(pj_range_indv_dir, str1), '{}pjRangeIter {}.csv'.format( pj_range_iter_dir, str1)
            pjs_p = [pj_vals_indv_p, pj_range_indv_p, pj_range_iter_p]
            self.iterator_netg(g, c, Alpha, Beta, Gamma, ver_com, ver_network, ver_net2, ver_indp, Ovs_p, quantile, pjs_p)
        ...
        f = open(log_dir+'log-PSO(8-14).txt', 'a')
